...
func (wh *AccountWarehouse) GetAccountKey(ctx context.Context, id string, ttl, maxTTL time.Duration, numKeys int, params *clouds.ResourceTokenCreationParams) (string, string, error) {
	...
	if k != nil {
		// TODO(cdvoisin): remove this if it doesn't / will never work.
...
func (wh *AccountWarehouse) ManageAccountKeys(ctx context.Context, project, account string, ttl, maxKeyTTL time.Duration, keysPerAccount int) (*iam.ServiceAccountKey, int, int, error) {
	...
	if match != nil {
		// TODO(cdvoisin): remove this matching stuff if it doesn't work (need to make delete an extra key below if can't reuse a key)
...
func (wh *AccountWarehouse) getBackingAccount(ctx context.Context, id string, params *clouds.ResourceTokenCreationParams) (string, error) {
	...
	if err == nil {
		// TODO(cdvoisin): verify there are no user_id->SA_account collisions.
...
func convertToPermanentErrorIfApplicable(err error, formattedErr error) error {
	...
	// TODO(cdvoisin): Extend this function's logic if other types of errors need
