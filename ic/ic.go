...
import (
	...
	compb "github.com/GoogleCloudPlatform/healthcare-federated-access-services/common/models"
	...
	pb "github.com/GoogleCloudPlatform/healthcare-federated-access-services/ic/api/v1"
)
...
var (
	...
	filterIDTokScope = map[string]bool{
		"openid":  true,
		"profile": true,
		// TODO(cdvoisin): remove these once DDAP BFF switches to use access token.
		"ga4gh":      true,
		"identities": true,
	}
...
func (s *Service) ConfigHistory(w http.ResponseWriter, r *http.Request) {
	...
	// TODO(cdvoisin): consider requiring an "admin" scope (modify all admin handlerSetup calls).
...
func (s *Service) ConfigReset(w http.ResponseWriter, r *http.Request) {
	// TODO(cdvoisin): probably should not be a GET, but handy for now on a browser...
	if r.Method != http.MethodGet {
		common.HandleError(http.StatusBadRequest, fmt.Errorf("request method not supported: %q", r.Method), w)
		return
	}
...
func (s *Service) Token(w http.ResponseWriter, r *http.Request) {
	...
	if typ == "authorization_code" {
		...
		// TODO(cdvoisin): match redirect_uri with the one in the auth code.
...
func (s *Service) Revocation(w http.ResponseWriter, r *http.Request) {
	...
	// TODO(cdvoisin): remove the RevocationRequest proto fields as GetParam() is needed instead.
...
func (s *Service) AcceptLogin(w http.ResponseWriter, r *http.Request) {
	...
	// TODO(cdvoisin): add security checks here as per OIDC spec.
...
func (s *Service) FinishLogin(w http.ResponseWriter, r *http.Request) {
	...
	// TODO(cdvoisin): id_token can be filled in by dbgap hack (this is to be replaced by OIDC flows).
...
func (c *config) Put(name string) error {
	if c.cfg.Version != c.input.Item.Version {
		// TODO(cdvoisin): consider upgrading older config versions automatically.
...
func (c *account) CheckIntegrity() (proto.Message, int, error) {
	// TODO(cdvoisin): add more checks for accounts here.
	return nil, http.StatusOK, nil
}
...
func (c *accountLink) CheckIntegrity() (proto.Message, int, error) {
	// TODO(cdvoisin): add more checks for accounts here (such as removing the primary email account).
	return nil, http.StatusOK, nil
}
...
func (s *Service) handlerSetup(tx storage.Tx, isAdmin bool, r *http.Request, scope string, item proto.Message) (*pb.IcConfig, *ga4gh.Identity, int, error) {
	...
	// TODO(cdvoisin): use only isAdmin by upgrading each handler to set this flag.
	path := common.RequestAbstractPath(r)
...
func (s *Service) getIdentity(r *http.Request, scope string, cfg *pb.IcConfig, tx storage.Tx) (*ga4gh.Identity, int, error) {
	// TODO(cdvoisin): move playground persona processing to a plug-in of some sort.
...
func (s *Service) getTokenIdentity(tok, scope, clientID string, tx storage.Tx) (*ga4gh.Identity, int, error) {
	...
	// TODO(cdvoisin): add more checks here as appropriate.
...
func (s *Service) accountToIdentity(acct *pb.Account, cfg *pb.IcConfig) *ga4gh.Identity {
	...
	for _, link := range acct.ConnectedAccounts {
		...
		// TODO(cdvoisin): consider skipping claims if idp=cfg.IdProvider[link.Provider] is missing (not <persona>) or idp.State != "ACTIVE".
...
func scopedIdentity(identity *ga4gh.Identity, scope, iss, subject string, iat, nbf, exp int64, aud []string, azp string) *ga4gh.Identity {
	...
	if !hasScopes("refresh", scope, matchFullScope) {
		// TODO(cdvoisin): remove this extra "ga4gh" check once DDAP is compatible.
...
func (s *Service) createAuthToken(subject, scope, provider, realm string, now time.Time, cfg *pb.IcConfig, tx storage.Tx) (string, error) {
	...
	// TODO(chaopeng): should set key id properly and sync with JWKS.
...
func (s *Service) createToken(identity *ga4gh.Identity, scope, aud, azp, realm string, now time.Time, ttl time.Duration, cfg *pb.IcConfig, tx storage.Tx) (string, error) {
	...
	if aud == "" || hasScopes("link", scope, matchPrefixScope) {
		...
		// TODO(chaopeng): we will change the token flow in phase 2, after that we just set the DAM needed to be audience here.
...
func populateAccountClaims(acct *pb.Account, id *ga4gh.Identity, provider string) {
	...
	if link == nil {
		...
		// TODO(cdvoisin): refresh some account profile attributes.
		link.Refreshed = now
		link.Revision++
	}
...
func (s *Service) getIssuerPublicKey(iss string, tx storage.Tx) (*rsa.PublicKey, error) {
	...
	if err != nil {
		// TODO(cdvoisin): Use OIDC JWKS to look up the public key.
...
func (s *Service) importFiles() error {
	// TODO(cdvoisin): FIXME do not check in this destructive code.
